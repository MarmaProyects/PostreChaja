                    ...
                    <div class="row bg-white my-4 p-3 card-shadow">
                        <div class="col-3 d-flex justify-content-center align-items-center py-3">
                            <a href="/productos/{{ $product->id }}">
                                <img src="data:image/jpg;base64, {{ $product->images()->first()->base64 }}" class="img-fluid" alt="{{ $product->name }}">
                            </a>
                        </div>
                        ...
                        <div class="col d-flex justify-content-between align-items-center fs-5">
                            ...
                            <form action="{{ route('productos.add_removeFavorite', ['product' => $product->id]) }}" method="POST" class="d-inline ms-2">
                                @csrf
                                <button type="submit" class="btn text-danger btn-lg"><i class="bi bi-trash"></i></button>
                            </form>
                        </div>
                    </div>
                    @endforeach
